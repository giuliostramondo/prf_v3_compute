...
typedef std::shared_ptr<FifoPushToPullSync> FifoPushToPullSyncPtr;
typedef std::shared_ptr<FifoPushToPushSync> FifoPushToPushSyncPtr;
 ...
 *
 */
enum StreamType {Stream_Is_Input, Stream_Is_Output, Stream_Is_Command, Stream_Is_Command_Echo};
...
public:
	// CONST
	size_t getNumberOfWordsFreeInFifo() const;
	size_t getNumberOfWordsInFifo() const;
	size_t getCapacity() const;
	bool isEmpty() const;
	bool isFull() const;
	const Data &peek() const;
	t_port_number getInputPortNumber() const {return m_input_port;}
	t_port_number getOutputPortNumber() const {return m_output_port;}
...
typedef std::shared_ptr<FifoCommandStream> FifoCommandStreamPtr;

/*
 * FifoCommandEchoStream is directly derived from FifoBackedPushToPullStream.
 * It is used for all command echo streams across the different memory control
 * groups.
 */

class FifoCommandEchoStream:
	public FifoBackedPushToPullStream
{
protected:
	/*Our proxy is our friend...*/
	friend class MemoryProxyCommandEchoSourceSync;
public:
	FifoCommandEchoStream (const std::string &stream_name_input, const std::string &stream_name_output, const FifoConfig &fifo_config):
		ManagerBlockSync(stream_name_input),
		FifoBackedPushToPullStream(stream_name_input,
				stream_name_output, Stream_Is_Command_Echo, fifo_config)
	{}
	bool canEcho(const Data command);
	void echoCommand(const Data command);
};

typedef std::shared_ptr<FifoCommandEchoStream> FifoCommandEchoStreamPtr;
...
protected:
	...
	FifoCommandEchoStreamPtr m_echo;
	bool   m_debug_flags;
...
public:
	...
	void setEchoStream(FifoCommandEchoStreamPtr echo) {m_echo = echo;}
	FifoCommandEchoStreamPtr getEchoStream() const {return m_echo;}
...
typedef std::shared_ptr<FifoInputStream> FifoInputStreamPtr;
...
typedef std::shared_ptr<FifoOutputStream> FifoOutputStreamPtr;
