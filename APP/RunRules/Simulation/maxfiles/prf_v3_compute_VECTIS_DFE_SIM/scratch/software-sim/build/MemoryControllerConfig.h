...
typedef std::shared_ptr<MemoryControllerConfig> MemoryControllerConfigPtr;
...
public:
	...
	// CONST Methods
	EnumBoardType getBoardType() const {return m_board_type;}
	size_t getMemoryBusBitWidth() const {return m_memory_bus_width;}
	size_t getStreamFIFOWordDepth() const {return m_stream_fifo_depth;}
	size_t getStreamFIFOBitWidth() const {return getMemoryBusBitWidth();}

	size_t getBurstWordSize() const {return m_burst_word_size;}
	size_t getBurstBitSize() const {return getBurstWordSize() * getMemoryBusBitWidth();}

	size_t getCommandFIFOWordDepth() const {return m_command_fifo_depth;}
	size_t getCommandFIFOBitWidth() const {return m_command_fifo_width;}

	uint64_t getMemoryByteSize() const {return m_mem_size_bytes;}
	...
	uint32_t getMaxMemoryBurstWordSize() const;
	uint32_t getMaxMemoryBusBitWidth() const;

	uint32_t getEccParityEngineBitSize() const {return m_ecc_parity_size_bits;}
	EnumMemCheckLevel getEccParityLevel() const {return m_mem_check_level;}

	bool equals(const MemoryControllerConfigParams & params) const;
	...
	static uint32_t getNumberOfBitsToEncodeValue(const uint32_t number);
	static uint32_t getMaxMemoryBurstWordSize(const EnumBoardType some_max_board);
	static uint32_t getMaxMemoryBusBitWidth(const EnumBoardType some_max_board);
};
