...
public:
	...
	friend std::ostream& operator<<(std::ostream& o, const varint_u<size> &v) {
		if(o.flags() & std::ios_base::hex) {
			...
			const int print_width = o.width();
			const char print_fill = o.fill();
			...
			bool leadingBits = false;
			bool leadingPad = false;
			if (num_top_bits != 0) {
				const int width = std::max(print_width - num_words * num_word_bits / 4, 0);
				if (0 == num_words || v.get_bits(num_words * num_word_bits, num_top_bits) != 0) {
					o << std::setw(width) << std::setfill(print_fill) << v.get_bits(num_words * num_word_bits, num_top_bits);
					leadingBits = true;
				} else if (width > 0) {
					o << std::setw(width) << std::setfill(print_fill) << "";
					leadingPad = true;
				}
			}

			for (int i = num_words - 1; i >= 0; i--) {
				const int width = leadingBits | leadingPad ? num_word_bits / 4 : std::max(print_width - i * num_word_bits / 4, 0);
				const char fill = leadingBits ? '0' : print_fill;
				if (leadingBits || 0 == i || v.get_bits(i * num_word_bits, num_word_bits) != 0) {
					o << std::setw(width) << std::setfill(fill) << v.get_bits(i * num_word_bits, num_word_bits);
					leadingBits = true;
				} else if (width > 0) {
					o << std::setw(width) << std::setfill(fill) << "";
					leadingPad = true;
				}
			}
	...
	void set_bits(int base, int length, uint64_t value) {
		...
		const uint64_t l_mask = l_length == s_chunk_bits ? 0 : ~(((uint64_t(1) << l_length) - 1) << l_offset);
