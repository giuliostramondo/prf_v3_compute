...
public:
	...
	// CONST
	uint32_t getAddressSize() const {return m_mem_cmd_start_addr_size;}
	uint32_t getPadAddressSize() const {return m_mem_cmd_pad_addr_size;}
	uint32_t getCountSize() const {return m_mem_cmd_count_size;}
	uint32_t getPadCountSize() const {return m_mem_cmd_count_pad_size;}
	uint32_t getIncrementSize() const {return m_mem_cmd_increment_size;}
	uint32_t getStreamSelectSize(const uint8_t number_of_streams) const {
		return getStreamSelectSizeAndPad(number_of_streams).first;
	}
	uint32_t getPadStreamSelectSize(const uint8_t number_of_streams) const {
	...
	// STATIC
	static uint32_t getAddress(const Data &command);
	static bool     getEchoFlag(const Data &command);
	static uint32_t getNumberOfBursts(const Data &command);
	static uint64_t getIncrement(const Data &command);
	static uint64_t getIncrementAndMode(const Data &command);
	static uint32_t getStreamSelect(const Data &command);
	static uint32_t getStreamSelectOneHotValue(const Data &command);
	static uint32_t getTag(const Data &command);
	static uint32_t getCommandWidth();
	static bool     isNullCommandNoStream(const Data &command) {return getStreamSelectOneHotValue(command) == 0;}
	static bool     isCmdMode(const Data &command) {return getNumberOfBursts(command) == 0;}
	static uint32_t getFlags(const Data &command);
	static Command  getCmd(const Data &command);
...
typedef std::shared_ptr<MemoryControllerCommand> MemoryControllerCommandPtr;
